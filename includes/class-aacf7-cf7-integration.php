...
class CF7_Integration {
	...
	public function init(): void {
		...
		// ✅ Validación servidor (CF7) para required
		add_filter('wpcf7_validate_adjuntos_cf7', [$this, 'validate_form_tag'], 10, 2);
		add_filter('wpcf7_validate_adjuntos_cf7*', [$this, 'validate_form_tag'], 10, 2);
	...
	public function render_form_tag($tag): string {
		...
		// ✅ FIX (1): evitar que primero se vea el CSS por defecto y luego cambie (FOUC)
		// Generamos un CSS inline simple desde los estilos guardados.
		$dynamic_css = $this->build_dynamic_css('#' . $instance_id, $styles);
		...
		if ($dynamic_css !== '') {
			$html .= '<style id="' . esc_attr($instance_id . '-aacf7-dynamic-inline') . '">' . $dynamic_css . '</style>';
		}
	...
	/**
	 * ✅ Validación servidor: si el campo es required y no hay archivo, CF7 debe bloquear el envío.
	 */
	public function validate_form_tag($result, $tag) {
		if (!is_object($tag)) {
			return $result;
		}
		...
		// Form ID actual (para mensaje personalizado)
		$form_id = 0;
		if (class_exists('\\WPCF7_ContactForm') && method_exists('\\WPCF7_ContactForm', 'get_current')) {
			$cf7 = \WPCF7_ContactForm::get_current();
			if ($cf7 && method_exists($cf7, 'id')) {
		...
		if (isset($_FILES[$name]) && is_array($_FILES[$name])) {
			// Single
			if (isset($_FILES[$name]['name']) && is_string($_FILES[$name]['name']) && $_FILES[$name]['name'] !== '') {
				$has_file = true;
			}
			// Multiple
			if (!$has_file && isset($_FILES[$name]['name']) && is_array($_FILES[$name]['name'])) {
		...
		if (!$has_file && method_exists($result, 'invalidate')) {
	...
	/**
	 * ✅ CSS dinámico server-side (reduce el "parpadeo" de estilos).
	 * Nota: no intenta replicar 100% todo tu JS, pero cubre lo principal.
	 */
	private function build_dynamic_css(string $rootSel, array $styles): string {
		if (!is_array($styles) || empty($styles)) {
			return '';
		}

		$container = isset($styles['container']) && is_array($styles['container']) ? $styles['container'] : [];
		$dropzone  = isset($styles['dropzone']) && is_array($styles['dropzone']) ? $styles['dropzone'] : [];
		$button    = isset($styles['button']) && is_array($styles['button']) ? $styles['button'] : [];
		$note      = isset($styles['note']) && is_array($styles['note']) ? $styles['note'] : [];
		$file      = isset($styles['file']) && is_array($styles['file']) ? $styles['file'] : [];

		$css = '';

		$css .= $rootSel . '{';
		if (!empty($container['bg'])) $css .= 'background:' . trim((string) $container['bg']) . ';';
		if (!empty($container['radius'])) $css .= 'border-radius:' . trim((string) $container['radius']) . ';';

		$bw = isset($container['border_width']) ? trim((string)$container['border_width']) : '';
		$bs = isset($container['border_style']) ? trim((string)$container['border_style']) : '';
		$bc = isset($container['border_color']) ? trim((string)$container['border_color']) : '';
		if ($bw !== '' || $bs !== '' || $bc !== '') {
			$css .= 'border-style:' . ($bs !== '' ? $bs : 'solid') . ';';
			$css .= 'border-width:' . ($bw !== '' ? $bw : '1px') . ';';
			$css .= 'border-color:' . ($bc !== '' ? $bc : '#dcdcdc') . ';';
		}

		if (!empty($container['box_shadow'])) $css .= 'box-shadow:' . trim((string) $container['box_shadow']) . ';';
		if (!empty($container['padding'])) $css .= 'padding:' . trim((string) $container['padding']) . ';';
		if (!empty($container['margin'])) $css .= 'margin:' . trim((string) $container['margin']) . ';';
		$css .= '}';

		$css .= $rootSel . ' .aacf7-title{';
		if (!empty($container['title_color'])) $css .= 'color:' . trim((string) $container['title_color']) . ';';
		if (!empty($container['title_size'])) $css .= 'font-size:' . trim((string) $container['title_size']) . ';';
		if (!empty($container['title_weight'])) $css .= 'font-weight:' . trim((string) $container['title_weight']) . ';';
		if (!empty($container['title_padding'])) $css .= 'padding:' . trim((string) $container['title_padding']) . ';';
		if (!empty($container['title_margin'])) $css .= 'margin:' . trim((string) $container['title_margin']) . ';';
		$css .= '}';

		$css .= $rootSel . ' .aacf7-dropzone{';
		if (!empty($dropzone['bg'])) $css .= 'background:' . trim((string) $dropzone['bg']) . ';';
		if (!empty($dropzone['radius'])) $css .= 'border-radius:' . trim((string) $dropzone['radius']) . ';';

		$bw = isset($dropzone['border_width']) ? trim((string)$dropzone['border_width']) : '';
		$bs = isset($dropzone['border_style']) ? trim((string)$dropzone['border_style']) : '';
		$bc = isset($dropzone['border_color']) ? trim((string)$dropzone['border_color']) : '';
		if ($bw !== '' || $bs !== '' || $bc !== '') {
			$css .= 'border-style:' . ($bs !== '' ? $bs : 'dotted') . ';';
			$css .= 'border-width:' . ($bw !== '' ? $bw : '1px') . ';';
			$css .= 'border-color:' . ($bc !== '' ? $bc : '#cccccc') . ';';
		}

		if (!empty($dropzone['box_shadow'])) $css .= 'box-shadow:' . trim((string) $dropzone['box_shadow']) . ';';
		if (!empty($dropzone['padding'])) $css .= 'padding:' . trim((string) $dropzone['padding']) . ';';
		if (!empty($dropzone['margin'])) $css .= 'margin:' . trim((string) $dropzone['margin']) . ';';
		$css .= '}';

		$css .= $rootSel . ' .aacf7-dropzone-text{';
		if (!empty($dropzone['text_color'])) $css .= 'color:' . trim((string) $dropzone['text_color']) . ';';
		if (!empty($dropzone['text_size'])) $css .= 'font-size:' . trim((string) $dropzone['text_size']) . ';';
		if (!empty($dropzone['text_weight'])) $css .= 'font-weight:' . trim((string) $dropzone['text_weight']) . ';';
		if (!empty($dropzone['text_padding'])) $css .= 'padding:' . trim((string) $dropzone['text_padding']) . ';';
		if (!empty($dropzone['text_margin'])) $css .= 'margin:' . trim((string) $dropzone['text_margin']) . ';';
		$css .= '}';

		$css .= $rootSel . ' .aacf7-dropzone-icon{';
		if (!empty($dropzone['icon_color'])) $css .= 'color:' . trim((string) $dropzone['icon_color']) . ';';
		if (!empty($dropzone['icon_size'])) $css .= 'font-size:' . trim((string) $dropzone['icon_size']) . ';';
		$css .= '}';

		$css .= $rootSel . ' .aacf7-btn{';
		if (!empty($button['padding'])) $css .= 'padding:' . trim((string) $button['padding']) . ';';
		if (!empty($button['margin'])) $css .= 'margin:' . trim((string) $button['margin']) . ';';
		if (!empty($button['radius'])) $css .= 'border-radius:' . trim((string) $button['radius']) . ';';

		$bw = isset($button['border_width']) ? trim((string)$button['border_width']) : '';
		$bs = isset($button['border_style']) ? trim((string)$button['border_style']) : '';
		$bc = isset($button['border_color']) ? trim((string)$button['border_color']) : '';
		if ($bw !== '' || $bs !== '' || $bc !== '') {
			$css .= 'border-style:' . ($bs !== '' ? $bs : 'solid') . ';';
			$css .= 'border-width:' . ($bw !== '' ? $bw : '1px') . ';';
			$css .= 'border-color:' . ($bc !== '' ? $bc : 'transparent') . ';';
		}

		if (!empty($button['bg'])) $css .= 'background:' . trim((string) $button['bg']) . ';';
		if (!empty($button['text_color'])) $css .= 'color:' . trim((string) $button['text_color']) . ';';
		if (!empty($button['text_size'])) $css .= 'font-size:' . trim((string) $button['text_size']) . ';';
		if (!empty($button['text_weight'])) $css .= 'font-weight:' . trim((string) $button['text_weight']) . ';';
		if (!empty($button['box_shadow'])) $css .= 'box-shadow:' . trim((string) $button['box_shadow']) . ';';
		$css .= '}';

		if (!empty($button['hover']) || !empty($button['text_hover'])) {
			$css .= $rootSel . ' .aacf7-btn:hover{';
			if (!empty($button['hover'])) $css .= 'background:' . trim((string) $button['hover']) . ';';
			if (!empty($button['text_hover'])) $css .= 'color:' . trim((string) $button['text_hover']) . ';';
			$css .= '}';
		}

		$css .= $rootSel . ' .aacf7-note{';
		if (!empty($note['color'])) $css .= 'color:' . trim((string) $note['color']) . ';';
		if (!empty($note['size'])) $css .= 'font-size:' . trim((string) $note['size']) . ';';
		if (!empty($note['weight'])) $css .= 'font-weight:' . trim((string) $note['weight']) . ';';
		if (!empty($note['margin'])) $css .= 'margin:' . trim((string) $note['margin']) . ';';
		if (!empty($note['padding'])) $css .= 'padding:' . trim((string) $note['padding']) . ';';
		$css .= '}';

		$css .= $rootSel . ' .aacf7-file-item{';
		if (!empty($file['bg'])) $css .= 'background:' . trim((string) $file['bg']) . ';';
		if (!empty($file['radius'])) $css .= 'border-radius:' . trim((string) $file['radius']) . ';';
		if (!empty($file['padding'])) $css .= 'padding:' . trim((string) $file['padding']) . ';';
		if (!empty($file['margin'])) $css .= 'margin:' . trim((string) $file['margin']) . ';';
		$css .= '}';
		...
		<div class="acf7-wrapper" data-aacf7-settings="1">
			...
			<div class="acf7-tab-content" id="tab-estilos">
				<div class="acf7-section">
					...
					<!-- (Aquí sigue exactamente tu HTML de estilos, no lo modifiqué) -->
