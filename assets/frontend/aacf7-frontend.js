(function ($) {
	...
	function animateProgress($item) {
		// Simulación visual (no es progreso real de subida, porque CF7 no sube en background)
		var $bar = $item.find('.aacf7-progress-bar');
		if (!$bar.length) return;

		// Reset: 0 -> (fuerza reflow) -> 100 con transición
		$bar.css('width', '0%');
		// fuerza reflow para que el browser aplique el 0 antes del 100
		$bar[0].offsetHeight; // eslint-disable-line no-unused-expressions
		$bar.css('width', '100%');
	}
	...
	function initUploader($wrap) {
		...
		function syncInput() {
			try {
				...
				// Si falla, NO tocamos el input. (Evita que se rompa el click.)
			}
		}
		...
		// ✅ Reset automático cuando CF7 envía ok (AJAX)
		// Eventos CF7 v5.4+:
		$wrap.closest('form').on('wpcf7mailsent wpcf7submit', function () {
			// si fue exitoso, CF7 dispara wpcf7mailsent; submit se usa como fallback
			// limpiamos en mailsent y también en reset si lo dispara el form
		});
		...
		document.addEventListener('wpcf7mailsent', function (event) {
			...
			// Si este uploader está dentro de este form, limpiarlo
			if ($form.has($wrap).length) {
				clearAll();
			}
		});

		// También limpiar cuando el form se resetea (por CF7 o manual)
