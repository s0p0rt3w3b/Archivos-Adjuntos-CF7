...
class CF7_Integration {
	...
	public function render_form_tag($tag): string {
		...
		// NOTA: En el siguiente cambio lo quitamos como acordamos.
		if ($delivery_mode === 'url') {
			$html .= '  <div class="aacf7-note">'
				. esc_html__('Modo URL: en el correo no se adjunta el archivo. Usa [aacf7_urls nombre] en Mail para imprimir enlaces (lo implementaremos en el siguiente paso).', 'archivos-adjuntos-cf7')
				. '</div>';
		} else {
			$html .= '  <div class="aacf7-note">'
				. esc_html__('Modo adjunto: el archivo se adjuntará al correo (se implementará en el siguiente paso).', 'archivos-adjuntos-cf7')
				. '</div>';
		}
	...
	public function render_editor_panel($form) {
		...
		$text_note  = (string) ($s['text_note'] ?? 'Máximo 5MB por archivo');
