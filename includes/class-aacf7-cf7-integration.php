...
class CF7_Integration {
	...
	public function render_form_tag($tag): string {
		...
		$html  = '';
		$html .= '<div class="aacf7-wrap" id="' . esc_attr($instance_id) . '" data-form-id="' . esc_attr((string) $form_id) . '" data-field-name="' . esc_attr($name) . '">';
		...
		$html .= '    <input type="file" class="aacf7-input" id="' . esc_attr($input_id) . '" name="' . esc_attr($name) . '"' . $required_attr . ' />';
		$html .= '  </div>';

		$html .= '  <div class="aacf7-note">' . esc_html__('Formatos permitidos y límites configurables por el administrador.', 'archivos-adjuntos-cf7') . '</div>';
	...
	private function get_current_form_id(): int {
		// Intento simple: si hay post en contexto.
		$post_id = get_the_ID();
